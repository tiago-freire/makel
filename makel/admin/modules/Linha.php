...
class Linha {
	...
	function lista_default($modulo,$registros=10,$pagina=1,$ordene="ID",$tipo="DESC",$url="") {
	...
	function getLast($limit=10,$ordem="",$tipo="ASC",$exclude="") {
		if ($limit)
			$lt = " limit $limit";
		if($exclude)
	...
	function getUltima() {
		$lista = self::getLast(1, "id", "DESC");
		if($lista) return $lista[0];
		return 0;
	}
	
	function getAll($pagina=1,$limite=10,$ordem="",$tipo="ASC",$url="") {
		if ($ordem)
			$ord = " order by $ordem $tipo";
		$consulta = "SELECT * FROM " . self::$table . $ord;
